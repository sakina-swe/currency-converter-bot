...
if (isset($update) && isset($update->message)) {
    ...
    $date = $message->forward_from_message_time ?? 'Not specified';
    ...
    if ($text != '/start' && is_numeric($text)) {

        $conversionType = $db->getState($chat_id);

        $input = explode('2', $conversionType);

        $originalCurrency = strtolower($input[0]);
        $targetCurrency = strtolower($input[1]);

        $result = $currency->convert($originalCurrency, $targetCurrency, $text);
        $db->clearTable();
...
if ($update->callback_query) {
    ...
    $booldb = $db->setState($chatId, $callbackData);

    $bot->http->post('sendMessage', [
        'form_params' => [
            'chat_id' => $chatId,
            'text' => 'Enter amount: ' . $booldb . $callbackData,
        ]
    ]);
}
...
//
//    $input = explode('2', $text);
//
//    $ccy_to_convert_from = strtolower($input[0]);
//    $ccy_to_convert_to = strtolower($input[1]);
////    $amount = $input[2];
//
//    $result = $currency->convert($chat_id, $ccy_to_convert_from, $ccy_to_convert_to, $input[2]);
//
//}